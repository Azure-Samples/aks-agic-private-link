...
param virtualNetworkName string = '${prefix}ServerVnet'
...
@description('Specifies the name of the private endpoint to the boot diagnostics storage account.')
param clientStorageAccountPrivateEndpointName string = 'ClientBlobStorageAccountPrivateEndpoint'
...
param bastionHostName string = '${prefix}ServerBastion'
...
param applicationGatewayName string = '${prefix}ApplicationGateway'
...
param natGatewayName string = '${prefix}NatGateway'
...
param clientVmSubnetPrivateEndpointNetworkPolicies string = 'Disabled'
...
param clientVmSubnetPrivateLinkServiceNetworkPolicies string = 'Enabled'
...
@description('Specifies the Bastion subnet IP prefix. This prefix must be within vnet IP prefix address space.')
param clientBastionSubnetAddressPrefix string = '192.168.1.0/24'
...
module aksNetwork 'aksNetwork.bicep' = {
  name: 'aksNetwork'
  params: {
...
module jumpboxVirtualMachine 'virtualMachine.bicep' = if (vmEnabled) {
  ...
  params: {
    vmName: vmName
    vmSize: vmSize
    vmSubnetId: aksNetwork.outputs.vmSubnetId
...
module aksManageIdentity 'aksManagedIdentity.bicep' = {
  ...
  params: {
    ...
    virtualNetworkName: aksNetwork.outputs.virtualNetworkName
    systemAgentPoolSubnetName: systemAgentPoolSubnetName
    userAgentPoolSubnetName: userAgentPoolSubnetName
    podSubnetName: podSubnetName
    apiServerSubnetName: apiServerSubnetName
...
module applicationGateway 'applicationGateway.bicep' = if (applicationGatewayEnabled) {
  ...
  params: {
    ...
    subnetId: aksNetwork.outputs.applicationGatewaySubnetId
    privateLinkSubnetId: aksNetwork.outputs.vmSubnetId
...
module aksCluster 'aksCluster.bicep' = {
  ...
  params: {
    ...
    applicationGatewayEnabled: applicationGatewayEnabled
    applicationGatewayName: applicationGatewayEnabled ? applicationGateway.outputs.name : ''
    keyVaultName: keyVault.outputs.name
    retentionInDays: logAnalyticsRetentionInDays
    workspaceId: workspace.outputs.id
    location: location
...
module clientStorageAccount 'clientStorageAccount.bicep' = {
...
module clientNetwork 'clientNetwork.bicep' = {
  name: 'clientNetwork'
  params: {
    virtualNetworkName: clientVirtualNetworkName
    virtualNetworkAddressPrefixes: clientVirtualNetworkAddressPrefixes
    vmSubnetName: clientVmSubnetName
    vmSubnetAddressPrefix: clientVmSubnetAddressPrefix
    vmSubnetPrivateEndpointNetworkPolicies: clientVmSubnetPrivateEndpointNetworkPolicies
    vmSubnetPrivateLinkServiceNetworkPolicies: clientVmSubnetPrivateLinkServiceNetworkPolicies
    bastionSubnetAddressPrefix: clientBastionSubnetAddressPrefix
    bastionHostName: clientBastionHostName
    bastionSubnetNsgName: 'ClientBastionSubnetNsg'
    storageAccountId: clientStorageAccount.outputs.id
    storageAccountPrivateEndpointName: clientStorageAccountPrivateEndpointName
    applicationGatewayId: applicationGateway.outputs.id
    applicationGatewayPrivateEndpointName: applicationGatewayPrivateEndpointName
    applicationGatewayPrivateLinkFrontendIPConfigurationName: applicationGateway.outputs.privateLinkFrontendIPConfigurationName
    privateDnsZoneName: domain
    privateDnsZoneARecordName: subdomain
    workspaceId: clientWorkspace.outputs.id
    retentionInDays: logAnalyticsRetentionInDays
    location: location
    tags: tags
...
module clientVirtualMachine 'virtualMachine.bicep' = {
  ...
  params: {
    vmName: clientVmName
    vmSize: clientVmSize
    vmSubnetId: clientNetwork.outputs.vmSubnetId
